...
export default function Electronics() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("http://localhost:8080/api/type/electronics");
 ...
 useEffect(() => {
  ...
  return (
    <div>
      <Navbar />
      <div className="bg-fade-electronics">
      </div>
      <div className="product-container-electronics">
        {items.map((item) => (
          <div key={item.id} className="product-box-electronics">
