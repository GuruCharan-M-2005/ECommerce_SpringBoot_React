...
const Cart = () => {
  ...
  const handleDecrement =  async(item) => {
    const res=await axios.put(`http://localhost:8080/api/cart/${item.id}/dec`,item);
    setTimeout(() => {
      addToCart();
    }, 100);
    // setCart(res.data);
  ...
  return (
    ...
    <div className="quantity">
      <button onClick={() => handleDecrement(item)} className="minus-but"><CgRemoveR /></button>
      <span>{item.count}</span>
      <button onClick={() => handleIncrement(item)}><CgAddR /></button>
    </div>
    <div className="cart-item-total"><LuIndianRupee className="rupee-cart" />{item.price * item.count}</div>
    <div>
      <button onClick={() => handleRemoveItem(item)} className="cart-remove">
        <CiCircleRemove />
      </button>
    </div>
  </div>
